...
int main(int argc, char **argv) {    
    ...
    // Read past the other scalar fields
    char * otherScalar = (char *)malloc(24);
    myFile->read(otherScalar, 24);

    // Read the reference names
    int32_t refNamesLen;
    myFile->read((char*)&refNamesLen, sizeof(refNamesLen));
    char * refNames = (char *)malloc(sizeof(char) * refNamesLen);
    myFile->read(refNames, sizeof(char) * refNamesLen);
    ...
    for ( int i = 0; i < numReferences; ++ i ) {
       ...
       if (numBins > 0) {
          if (!isFirst)
             ...
             isFirst = false;
          cout << "#" << i << "\t" << refNames << "\t" << numLinearOffsets * 16384;
          int roundsToAdd;
